...
def generate_excel(ticker):
    ...
    # Debug: Print first few rows to check the DataFrame
    print("First few rows of balanceSheet: ", balanceSheet.head())

    print(balanceSheet.columns)
  # Optional: If the issue is due to extra whitespaces in column names, you can strip them
   ...
   # Now try the operation again
    try:
        ...
        liability_growth = (
            balanceSheet["totalCurrentLiabilities"].pct_change()).mean()
    except KeyError as e:
        print(f"Error: {e}. Column not found.")

    asset_growth = (balanceSheet["totalCurrentAssets"].pct_change()).mean()
    liability_growth = (balanceSheet["totalCurrentLiabilities"].pct_change(
    )).mean()
    ...
    wacc = weightDebt * interest_rate * \
        (1 - tax_rate) + weightEquity * costEquity
    print("WACC:", wacc)
    ...
    request = requests.get("https://api.sec-api.io/float?ticker=" + ticker.upper(
    ) + "&token=37a574d69a6bf9a82714b225b153d1a8d3a4cbcb1309d930f10799c2948f8684")
...
@app.route('/download<ticker>')  # Note: corrected URL parameter syntax
def download(ticker):
